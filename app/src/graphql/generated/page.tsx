...
import {
  getApolloClient,
  ApolloClientContext,
} from 'src/lib/withPrivateApollo';

export async function getServerPageAggregateFormTypes(
  options: Omit<
    Apollo.QueryOptions<Types.AggregateFormTypesQueryVariables>,
    'query'
  >,
  ...
  const data = await apolloClient.query<Types.AggregateFormTypesQuery>({
    ...options,
    query: Operations.AggregateFormTypesDocument,
  });
...
export const useAggregateFormTypes = (
  optionsFunc?: (
    ...
    Types.AggregateFormTypesQuery,
    Types.AggregateFormTypesQueryVariables
  >
) => {
...
export type PageAggregateFormTypesComp = React.FC<{
  data?: Types.AggregateFormTypesQuery;
  error?: Apollo.ApolloError;
}>;
export const withPageAggregateFormTypes =
  (
    optionsFunc?: (
      router: NextRouter
    ) => QueryHookOptions<
  ...
  (WrappedComponent: PageAggregateFormTypesComp): NextPage =>
  (props) => {
    const router = useRouter();
    const options = optionsFunc ? optionsFunc(router) : {};
    const { data, error } = useQuery(
      Operations.AggregateFormTypesDocument,
      options
    );
...
export async function getServerPageFormVideoUrl(
  ...
  const data = await apolloClient.query<Types.FormVideoUrlQuery>({
    ...options,
    query: Operations.FormVideoUrlDocument,
  });
...
export const useFormVideoUrl = (
  optionsFunc?: (
    ...
    Types.FormVideoUrlQuery,
    Types.FormVideoUrlQueryVariables
  >
) => {
  const router = useRouter();
  const options = optionsFunc ? optionsFunc(router) : {};
...
export type PageFormVideoUrlComp = React.FC<{
  data?: Types.FormVideoUrlQuery;
  error?: Apollo.ApolloError;
}>;
export const withPageFormVideoUrl =
  (
    optionsFunc?: (
      router: NextRouter
    ) => QueryHookOptions<
  ...
  (WrappedComponent: PageFormVideoUrlComp): NextPage =>
  (props) => {
    const router = useRouter();
    const options = optionsFunc ? optionsFunc(router) : {};
...
export async function getServerPageForms(
  options: Omit<Apollo.QueryOptions<Types.FormsQueryVariables>, 'query'>,
  ctx: ApolloClientContext
) {
  const apolloClient = getApolloClient(ctx);

  const data = await apolloClient.query<Types.FormsQuery>({
    ...options,
    query: Operations.FormsDocument,
  });
...
export const useForms = (
  optionsFunc?: (
    router: NextRouter
  ) => QueryHookOptions<Types.FormsQuery, Types.FormsQueryVariables>
) => {
  const router = useRouter();
  const options = optionsFunc ? optionsFunc(router) : {};
  return useQuery(Operations.FormsDocument, options);
};
export type PageFormsComp = React.FC<{
  data?: Types.FormsQuery;
  error?: Apollo.ApolloError;
}>;
export const withPageForms =
  (
    optionsFunc?: (
      router: NextRouter
    ) => QueryHookOptions<Types.FormsQuery, Types.FormsQueryVariables>
  ) =>
  (WrappedComponent: PageFormsComp): NextPage =>
  (props) => {
    const router = useRouter();
    const options = optionsFunc ? optionsFunc(router) : {};
...
export const withPageUsers =
  ...
  (WrappedComponent: PageUsersComp): NextPage =>
  (props) => {
    const router = useRouter();
    const options = optionsFunc ? optionsFunc(router) : {};
